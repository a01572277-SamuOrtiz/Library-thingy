...
Loan::Loan(string i, string m, string bDate) : isbn(i), memberId(m), borrowDate(bDate), returnDate("") {}
...
{
    cout << "Loan - ISBN: " << isbn << " | Member ID: " << memberId
         << " | Borrowed: " << borrowDate << " | Returned: "
         << (returnDate == "" ? "NOT RETURNED" : returnDate) << endl;
}